...
import dao.design.hibernate.IFuncionDAO;
...
public class FuncionDAO implements IFuncionDAO{
    ...
    public List<Funcion> filterBySituacion(boolean situacion) {
        ...
        try{
            ...
            Query query = session.createQuery("FROM Funcion S WHERE S.situacion = :situacion");
    ...
    public List<Funcion> findAll() {
        ...
        try{
            tx = session.beginTransaction();
            Query query = session.createQuery("FROM Funcion");
            result = query.list();
            tx.commit();
        }catch(HibernateException he){
            if(tx!=null)tx.rollback();
